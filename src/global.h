...
private:
  ...
  void setPriamry(const std::string& attrName);
  void setUnique(const std::string& attrName);
...
public:
  Attribute(const std::string& n, DATA_TYPE t, size_t l, size_t f):
    m_name(n), m_type(t), m_length(l), m_flag(f) {assert( isIndex() == false );}
...
//    m_name(n), m_type(t), m_length(l) {}
  ...
  void setPrimary() { m_flag |= 0x01; }
  void setUnique() { m_flag |= 0x02; }
...
class Query {
 public:
  Query(const std::string& tableName, const std::vector<Condition>& vtCon)
      : m_tableName(tableName), m_condition(vtCon) {}
  Query() {}
...
class Condition {
 public:
  Condition(const std::string& attrName, OP_TYPE opType,
...
inline void vtSplice(std::vector<int>& vtOut, const std::vector<int>& tmp)
{
