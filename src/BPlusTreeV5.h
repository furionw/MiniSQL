...
class BPlusTree {
 ...
 public:
  ...
  // called by Index::selectTuple
  std::vector<int> find(const Condition& condition) const;
  // for dbg
  void print() const;
  // ctor, if file doesn't exist, createRootOnEmpty()
  BPlusTree(const std::string& tableName, const Attribute& attr);
  // destructor, write m_valuePtr back to the first block(root block)
  ~BPlusTree();
private:
  ...
  KEY getLeastKey(const Node& node) const; // new added in 13-11-06
...
BPlusTree<KEY>::BPlusTree(const std::string& tableName, const Attribute& attr):        
  m_fileName( tableName ), m_length( attr.m_length ),
...
{
  BufferMan::instance().pin(m_fileName, 0);
  return  blockToNode(0);
}


template<typename KEY>
typename BPlusTree<KEY>::Node BPlusTree<KEY>::find(const Node& node,
    const KEY& key) const {
  return node.isLeaf() ? node : findInInner(node, key);
}

template<typename KEY>
typename BPlusTree<KEY>::Node BPlusTree<KEY>::findInInner(const Node& node,
    const KEY& key) const {
  auto itKey = node.m_key.begin();
  auto itChild = node.m_child.begin();
  for(; itKey != node.m_key.end(); itKey ++, itChild++)
    if(key < *itKey) return find( blockToNode(*itChild), key );
...
template<typename KEY>
void BPlusTree<KEY>::insert(const std::string& keyStr, const int pointer)
    const {
  ...
  if ( leaf.hasEnoughSpace(m_maxSize - 1) ) // 空间足够
  {
    insertInLeafWithNewKey(leaf, key, pointer);
    nodeToBuffer( leaf );                                  // 将leaf写回buffer
  } else
  {                                                                     
    insertInLeafWithNewKey(leaf, key, pointer);
    KEY midKey;
...
template<typename KEY>
void BPlusTree<KEY>::updateChild(const BPlusTree<KEY>::Node& parent,
    Node& child) const {
...
void BPlusTree<KEY>::unpackKey(typename BPlusTree<KEY>::Node& node, const std::string& content, const int listSize) const
{
  ...
  {
    node.m_key.push_back( stringToKey( content.substr(cur,  m_length).c_str() ) ); // c_str(), re-added in 2013-11-07
    cur += m_length;
  }
}
...
template<typename KEY>
void BPlusTree<KEY>::borrowFromLeft_inInner(typename BPlusTree<KEY>::Node& left,
    Node& right, const KEY& oldMidKey) const {
  ...
  left.m_child.pop_back();
  // Added 2016-11-12
  auto child = blockToNode(right.m_child.front());
  updateChild(right, child);
}
...
template<typename KEY>
KEY BPlusTree<KEY>::getLeastKey(const typename BPlusTree<KEY>::Node& node) const {
  return node.isLeaf()
      ? node.m_key.front()
      : getLeastKey(node.m_child.empty() == false
          ? blockToNode(node.m_child.front())
          : blockToNode(node.m_last));
}
...
template<typename KEY>
void BPlusTree<KEY>::borrowFromRight_inInner(typename BPlusTree<KEY>::Node& left,
    Node& right, const KEY& oldMidKey) const {
  left.m_key.push_back( oldMidKey );
  left.m_child.push_back( left.m_last );
  left.m_last = right.m_child.front();
  ...
  right.m_child.pop_front();
  // Added 2016-11-12
  auto child = blockToNode(left.m_last);
  updateChild(left, child);
}
