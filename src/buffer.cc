...
{
  int blockNum = File::readLast( fileName, strOut );
  if ( isInBuffer(fileName, blockNum) )
    readBlock(fileName, blockNum, strOut);                 // 在buffer中，此前从文件里读的File::readLast(..., strOut)是不正确的，从buffer中重读一次
  return blockNum;
}
...
void BufferMan::pin(const string& fileName, const int blockNum)
{
  m_map[ getKey(fileName, blockNum) ] -> setPin();
}
